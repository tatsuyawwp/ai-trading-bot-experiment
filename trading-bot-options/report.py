...
expectancy, profit factor, MFE/MAE, per-contract breakdown, and equity
drawdown. Read-only, safe to run anytime; does not touch the bot.

Same framework as experiments/trading-bot/report.py (crypto), ported
for the options (buy-only calls/puts) paper account. Realized/unrealized
P&L here is already premium-relative (Alpaca's unrealized_plpc for an
option position is cost-basis-relative, i.e. % change in premium), so
no extra normalization is needed vs. the equities/crypto version.
"""
...
def print_report():
    rows = load_rows()
    print("=== Paper Trading Report (Options, buy-only) ===")
    if not rows:
        print("No data yet.")
        return
    ...
    if orphaned_sells:
        print(f"\nNOTE: {orphaned_sells} sell fill(s) had no matching buy and were excluded from trade stats.")
    closed = [t for t in trades if t.is_closed]
    open_trades = [t for t in trades if not t.is_closed]

    print("\n--- Trade stats (closed contracts) ---")
    if not closed:
        ...
        gross_profit = sum(t.realized_pl_pct * t.entry_price * 100 for t in wins)
        gross_loss = sum(-t.realized_pl_pct * t.entry_price * 100 for t in losses)
        ...
        print(f"Expectancy: {expectancy_pct:+.2%} of premium per trade")
        print(f"Profit factor: {profit_factor:.2f}")

        print("\n--- MFE/MAE per closed trade ---")
        for t in closed:
            print(f"{t.symbol}: entry {t.entry_time.isoformat()} @ ${t.entry_price:.2f} -> exit {t.exit_time.isoformat()} "
    ...
    if open_trades:
        ...
        for t in open_trades:
            ...
            print(f"{t.symbol}: opened {t.entry_time.isoformat()} @ ${t.entry_price:.2f} | MFE so far {mfe} | MAE so far {mae}")
